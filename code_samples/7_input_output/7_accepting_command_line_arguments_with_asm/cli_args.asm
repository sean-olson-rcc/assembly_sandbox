...
;			r13:	argument pointer (char * argv[])
...
_start:
	...
	; with pure assembly, the arguments will be on the stack, not in registers

	; mov	r12,	[rsp]				; argc
	; lea	r13,	[rsp + 8] 	; assign the the first address pointer to r13
	...
	; 	jl main_loop_body		; if the current index is less than the number of arguments, jump to main_loop_body

	; 	jmp main_loop_done	; otherwise jump to the main_loop_done
	; ;--------------	
	; main loop body
	; main_loop_body:

		;--------------
		; print the next referenced argument and move the cursor to the next line
		; mov	rdi,	[r13]
		; mov	rsi,	FD_STDOUT
		; call print_null_terminated_string
		; call crlf
